...
void SLDC_plot_y_data(float y_data){
...
int buff=0U;
	...
	buff=y_data*1000;
	 ...
	 SLCD_Engine_Show_Num(&slcdEngine,inty[0],0,1);
	 SLCD_Engine_Show_Num(&slcdEngine,inty[1],1,1);
	 SLCD_Engine_Show_Num(&slcdEngine,inty[2],2,1);
	 SLCD_Engine_Show_Num(&slcdEngine,inty[3],3,1);
	 //decimal after first digit
	 SLCD_Engine_Show_Icon(&slcdEngine, 28, 1);
	}


	 //SLCD_Engine_Show_Icon(&slcdEngine, 29, 1);  decimal after second digit
...
int main(void)
{
    ...
    PRINTF("\r\nSLCD FXLS8974 Accelerometer Demo Begins.\r\n");
    ...
    dataScale =FXLS8974_Init(&fxlsHandle);
...
int count=100;
    while (1)
    {
    		y_data=FXLS8974_get_data(&fxlsHandle,dataScale, count);
    		count++;
    		if(count>100){
    			SLDC_plot_y_data(y_data);
    			count=0;
    		}
